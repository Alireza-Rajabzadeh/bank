...
class TransactionController extends Controller
{
    ...
    function __construct(TransactionService $transaction_service, AccountService $account_service, CardService $card_service, SmsService $sms_service)
    {
    ...
    function doTransaction(DoTransactionRequest $request)
    {
        ...
        $request_data['tansaction_ammount_and_wage'] = $request_data['ammount'] + env("BANK_WAGE", 500);
        ...
        try {
            DB::transaction(function () use ($request_data) {
                ...
                $this->account_service->decreaseCredit($request_data['origin_account'], $request_data['tansaction_ammount_and_wage']);
        ...
        $this->sms_service->decreaseMessage($request_data['origin_account']->user()->first(), $request_data['tansaction_ammount_and_wage']);
        $this->sms_service->increaseMessage($request_data['destination_account']->user()->first(), $request_data['ammount']);
